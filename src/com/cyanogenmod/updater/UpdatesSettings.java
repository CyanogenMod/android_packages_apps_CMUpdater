...
public class UpdatesSettings extends PreferenceActivity implements OnPreferenceChangeListener {
        ...
        if (mEnqueue != -1) {
            ...
            if (c == null) {
                ...
                if (c.moveToFirst()) {
                    ...
                    String status = c.getString(c.getColumnIndex(DownloadManager.COLUMN_STATUS));
                    if (lFile != null && status.equals(DownloadManager.STATUS_SUCCESSFUL)) {
    ...
    protected void stopDownload() {
        ...
        builder.setPositiveButton(R.string.dialog_yes, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                ...
                mUpdateHandler.removeCallbacks(updateProgress);
                mPrefs.edit().putLong(Constants.DOWNLOAD_ID, -1).apply();
                mEnqueue = -1;
                mFileName = null;
                mDownloading = false;
                Toast.makeText(UpdatesSettings.this, R.string.download_stopped, Toast.LENGTH_SHORT).show();
            }
        });
    ...
    private void downloadCompleted(long downloadId, String fullPathname) {
        ...
        if (pref != null) {
            ...
            if (DEBUG)
                Log.d(TAG, "Checking MD5 for " + fullPathname + " against " + downloadedMD5);

            // Start the MD5 check of the downloaded file
            if (MD5.checkMD5(downloadedMD5, destinationFile)) {
                // We passed the check
                // Set the preference style and start the update
                pref.setStyle(UpdatePreference.STYLE_DOWNLOADED);
                startUpdate(ui);
            } else {
                // We failed the check
                ...
                pref.setStyle(UpdatePreference.STYLE_NEW);
                Toast.makeText(UpdatesSettings.this, R.string.md5_verification_failed, Toast.LENGTH_LONG).show();
            }
        }
    }
    ...
    private void refreshPreferences() {
        if (mUpdatesList != null) {
            ...
            if (!mLocalUpdates.isEmpty()) {
                ...
                for (String fileName : mLocalUpdates) {

                    // TODO: the found match should not just check if the file exists but also that is
                    // was fully downloaded, once this is done, enable the NotificationClickReceiver code
                    foundMatch = fileName.equals(mFileName);
                    UpdatePreference up = new UpdatePreference(this, null, fileName,
