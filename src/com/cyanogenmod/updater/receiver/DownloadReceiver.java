...
public class DownloadReceiver extends BroadcastReceiver{
    ...
    private String fetchDownloadPartialPath(Context context, long id) {
        Query query = new Query();
        query.setFilterById(id);
        Cursor c = ((DownloadManager)
                context.getSystemService(Context.DOWNLOAD_SERVICE)).query(query);
        try {
            if (!c.moveToFirst()) {
                c.close();
                return null;
    ...
    private int fetchDownloadStatus(Context context, long id) {
        Query query = new Query();
        query.setFilterById(id);
        Cursor c = ((DownloadManager)
                context.getSystemService(Context.DOWNLOAD_SERVICE)).query(query);
